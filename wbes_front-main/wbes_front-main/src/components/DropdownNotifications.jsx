...
function DropdownNotifications({ align }) {
  ...
  useEffect(() => {
    async function fetchNotifications() {
      try {
        const response = await fetch('http://localhost:5000/api/notifications'); // Replace with your API endpoint
              ...
              notifications.map((notification) => (
                <li key={notification.id} className="border-b border-gray-200 dark:border-gray-700/60 last:border-0">
                  <Link
                    ...
                    onClick={() => setDropdownOpen(!dropdownOpen)}
                  >
                    <span className="block text-sm mb-2">📣 <span className="font-medium text-gray-800 dark:text-gray-100">{notification.title}</span> {notification.message}</span>
                    <span className="block text-xs font-medium text-gray-400 dark:text-gray-500">{new Date(notification.date).toLocaleDateString()}</span>
                  </Link>
                </li>
              ))
            ) : (
              <li className="py-2 px-4 text-sm text-gray-500 dark:text-gray-400">No notifications</li>
            )}
          </ul>
        </div>
      </Transition>
